...
; MiscDeclarations
	...
	; Render Data
	currentScreenIndexesChanged : var #400 ; Current max is forty by pushing 39 boxes into yourself
	...
	;TODO:
	ButtonArray: var #20
	NumButtonsInScene: var #1

	;TODO
	ObjectArray: var #1200  ; stores pointers in ObjectPool  (Former BehaviourLayer)
	ObjectPool: var #2000   ; stores Constructed Objects
	ObjectPoolPointer: var #1

		Static ObjectPoolPointer , #ObjectPool

	;TODO:
	TickOnAllFramesArray: var #100   ; Words are 16 bits. First five are id for the JumpDict Last 11 are position on screen.
	TickOnAllFramesJumpDict: var #32   ; 
...
ScreenRenderIndex:   ; < , ScreenIndex>
	...
	; call AccesStringIndex
		...
		loadn r4, #255
		and r2, r2, r4    ; Removes Object ID
...
DebugPrintValue:  ; <r0=valor, r1=posição>
    push r0
    push r1
    push r2
    push r3
    ...
    mod r3, r0, r2  ; dígito
    loadn r2, #48   ; '0'
    add r3, r3, r2  ; converter para ASCII
...
;------------ Math
	...
	Exp: ; <base, Exponent>  No floating points;   Does not handle overflow
		...
		loadn r3, #0
		loadn r4, #1
		;while e bigger than 0
		
		Exp_loop:
			cmp r1, r3 
			jeq Exp_exitloop:

			and r5, r4, r3  ; get last bit of exponent
			cmp r5, r3   ; is last bit zero
			jeq Exp_skiponce

				mul r0, r0, r2 ; mul result by base
				mul r2, r2, r2 ; square base
				shiftr0 r1, #1 ; div exp by 2
	...
	NegToPos:   ;<Negative Number> / <Positive Number>
...
;---------- Bit Manipulation
;
;    16 bits make a word, that is the adressabe memory
;
;

	SetBit: ; <MemoryAddress, Bit>
	...
	MapBitID: <id> ; Maps 0 to 1000000000000000, 1 to 0100000000000000, etc
		...
		;MapBitSequence:
			...
			and r5, r1, r3  ; get last bit of id
			cmp r5, r3   ; if 1
			jeq MapBit_skipOne

				mul r0, r0, r2 ; shift
				mul r2, r2, r2 ; square shift
				shiftr0 r1, #1 ; shift id
			...
			and r5, r1, r3  ; get last bit of id
			cmp r5, r3   ; if 1
			jeq MapBit_skipThree

				mul r0, r0, r2 ; shift
				mul r2, r2, r2 ; square shift
				shiftr0 r1, #1 ; shift id

				MapBit_skipThree: ; else
			...
			jeq MapBit_skipFour
				...
				MapBit_skipFour: ; else

		rts

	MapBitValue:    ; TODO:	

	InsertBits: ; <r0 , BitMask, bits>    r0 is the target
	...
	ExtractBits: ; <r0, Bitmask>
		
		and r0, r1, r0 ; actualy trivial doesnt need a function
...
;
;
	; IF IDE is built in the future error checking could be removed for Release Code

	CallError: 

		; retreive Message from ERRORID or just print it
	...
	CheckOverFlow: ; <ERRORID, Size, Buffer, BufferSize, BufferPointer >  Constructor can use to check for overflow in objectpool. If yes, call error. 

		;r0 = ERRORID
		;r1 = Size of Object

		add r2, r1, r4  ; Pointer + Size
		add r3, r2, r3  ; Buffer + BufferSize

		cmp r2, r3
		jle CheckOverFlow_END:
...
;
;		Static ObjectPoolSize , #2000
;
;	ObjectIDArray: var #256
;   ObjectIDArrayFragments: var #50
;	ObjectIDArrayFragmentsPointer : var #1
;
	...
	; Can be expanded to allow for dynamic objects and memory cleanup, not curently needed
		...
		ObjectPoolReserve:   ; <Lenght> / < , , RootMemoryAddres>
			...
			ObjectPoolReserve_loop:
				...
				sub r5, r3, r2
				cmp r5, r0
				jgr ObjectPoolReserve_MemReserve
		...
		ObjectIDReserve:    ; < > / < , IDMemAddres>
			...
			loadn r0, #1   
			loadn r1, #ObjectIDArrayFragments
			loadn r4, #0
			...
			ObjectIDReserve_loop:
				...
				; check if we reched the end
				cmp r3, r4
				jeq ObjectIDReserve_IDReserve

				; check for free IDs
				sub r5, r3, r2
				cmp r5, r4    ; if size = 0
				store ObjectIDReserve_IDCleanUp, r4
				jgr ObjectIDReserve_IDReserve ; if size > 0

				jmp ObjectIDReserve_loop

			; put it there and update frags and ID

			ObjectIDReserve_IDReserve: 

				; r2, r3  | end of frag , start of frag

				dec r1 
				inc r2
				storei r1, r2     ; r2 - new end of frag
		...
		; Private
		FragmentMerger: ; <*Array>

			; suposes only one object has been currentScreenIndexesChanged
			; can be sped if needed up by passing a pointer to where the data ends ; but realisticaly we do not expect too many fragmenets
		...
		ObjectPoolDeAllocate

			; Remove ID and updates fragments
...
; Game Objects 
;
;	Anything that must be interacted with. To reduce some bloat, things are not fully agregated
;	
;	Just Define a constructor and a behaviour function. Constructor will be run by the LevelDecoder when the character is found. 
;	Make sure to assing it
;


	; Background Prop Functions
		...
		; Goal
			...
			IsPlayerOnGoal:     ; <index>

				push r1

				load r1, playerPos
				cmp r0, r1
				jne IsPlayerOnGoal_No
		...
		; Conveyour Belt
			...
			;Constructor_Conveyour:
			;	rts
			; Use DoNothing

			ConveyourPush:    ; <Index, Direction>
				...
				call checkPushMovement; <New Position, Old Position>  ; i think this will work with the player too 

				pop r2
				rts

		; Button   <r7 = ID>   ; ID MustBePassedByTheCallBehavior

			; A button can have diferent IDS. it is the bit in the 16 bit word it will represent. 
			; this id is stored in the ObjectPool, and our pointer to it is in our position on the ObjectLayer 

			; the button will have two words of data, (properties and position) standard object data for my game and (Bitmask)
			...
			ButtonCheck:
				...
				; If on top of its sprite is another object, sey the bit acording to its ID to one

				; get its position from itself on the ObjectPool
				; check what is in PropLayer
				; Set its bit acordinly
...
; Level Data: Var #5  ;<HUD, Props, Background, Behavior, Topology, ButtonLayout>
